...
default answered_questions2 = []
default answered_questions3 = []
...
define audio.glass = "music/brokenglass.mp3"
...
"
    ...
    u "Потом со временем мы восстанавливаем свои воспоминания начиная с самого детства, прорабатываем свои травмы в психотерапиях.." 
...
"
    ...
    u "Надо бы включить свет и осмотреть комнату, может вспомню что-либо."
    hide damian322
    scene bg roomday
    with dissolve
...
label room_questions:
    menu:
        ...
        "Тумбочка" if 1 not in answered_questions1:
            ...
            u "В верхнем выдвижном ящике лежат ножницы и парочку карандашей"
            u "Итак, а в нижнем выдвижном ящике должны лежать три моих комплекта чистой одежды, четвертый на мне"
    ...
    u "Абсолютно у всех закрыты двери, как принято тут."
    ...
    u "Меня затошнило когда я снова подумал j крови"
    hide damian212
...
#концовка - демиан слышит шаги маартен
    show damian122
...
label coridor:
    scene bg coridor
    with dissolve
    d "Я быстро прошмыгнул через коридор дормитория, чтобы не видеть весь этот ужас"
    d "Я оказался в абсолютно темной месте, где горели красным только датчики пожарной сигнализации и мой небольшой переносной светильник"
    d "Тут также был стойкий запах крови, меня уже тошнит от него"
    jump coridor_questions

    return

label coridor_questions:
    menu:
        d "Надо бы осмотреться.."
#        "Процедурный кабинет":
#            jump coridor_questions
...
#        'Дверь с надписью "ТОЛЬКО ДЛЯ МЕДПЕРСОНАЛА"':
#            jump coridor_questions    

    
label kitchen_intro:
    d "Каждый раз когда мы туда приходим, там уже заготовлена еда в подносах для каждого пациента, в зависимости от его предпочтений."
    d "Каждый день еда меняется, но повторяется каждую неделю. Больше всего мне нравится пятница из-за творожной массы, которую мне кладут"
    d "После еды мы все выбрасываем остатки еды, а один дежурный из нас кладет всю посуду в посудомойку"
    d "По всей видимости, нам хотят так привить взаимосогласие и уважение друг к другу."
    "Войти в кухню?"
    menu:
...
label kitchen:
    scene bg kitchen
    with dissolve
    d "На кухне порядок как и всегда, но на столе пусто. Обычно вечером тут обязательно сидит кто-то, к примеру, Маартен с Донован"
    d "Я не особо общаюсь с другими пациентами, но заметил что некоторые из них уже сдружились между собой"
    d "Наверное это делает легче пребывание здесь"

    show damian121 

    d "Итак, что тут у нас"

    hide damian121

    jump kitchen_questions
    return

label kitchen_questions:
    menu:
        "Столешницы" if 0 not in answered_questions3:
            d "На столешницах лишь пару полных мисок со снеками"
            d "Нам часто оставляют эти миски на кухне, так что можно почитать тут книги свободное время и перекусывая чем-либо"
            $ answered_questions3.append(0)
            jump kitchen_questions
        "Стол" if 1 not in answered_questions3:
            d "Просто пустой стол.. Погодите.."
            d "На полу тут какие то листы бумаги похожие на отчеты"
            d "Похоже профессор Кипарис опять обронила один из листов ежедневного отчета для руководства и свои личные пометки там"
            d "Содержимое там ни для кого не секрет, так что об этом никто не беспокоится"
            $ answered_questions3.append(1)
            jump kitchen_questions
        "Холодильник" if 2 not in answered_questions3:
            d "На самом холодильнике несколько рисунков испытуемых, которые мы делали на одном из занятий"
            d "Некоторые из рисунков довольно детализированы и интересно их рассматривать"
            d "В самом холодильнике пару стеклянных бутылок соков и фрукты, которые можно нарезать самим"
            $ answered_questions3.append(2)
            jump kitchen_questions
        "Раковина" if 3 not in answered_questions3:
            d "Посудомойка пустая, а вот раковина.."
            d "Там лежат ножи?"
            d "Я думал, что на кухне лишь парочку ножа есть, которые не особо острые во избежании самоповреждений пациентов"
            d "Но тут пять ножей"
            d "Все они чистые и с парой каплями воды. Наверняка их помыли несколько часов назад и за это время они успели высохнуть"
            d "Думаю, возьму один из них, может пригодиться"
            $ answered_questions3.append(3)
            jump kitchen_questions        
    d "Итак, я все осмотрел, но что-то мне подсказывает что я что-то пропустил"
    d "Ах, точно, верхние шкафы" 
    d "Стоит ли их проверить?.." 
    menu:
        "Да":
            jump fifth_ending
        "Нет":
